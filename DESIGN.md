...
the test suite with it. A current host result reads 41 of 44 rather than a
...
the seven that depend on `-enable-private-imports`, which matters most, since
that is a second undocumented frontend option. The already-suspended async
case was additionally run by itself on the host under Xcode 26.2 and 26.5; it
passes there too. Nothing suggests the older rows would differ, but a row is
not expanded until the full matrix is re-run.
...
installed locally too, and its full run is recorded above. The CI row remains
useful because it was measured independently and because the macOS 15 target
...
Simulator and on both targets under Beta 6. Under Xcode 26.6 it again returned
the new value on the host and produced SIGSEGV on the Simulator. The checked-in
result files record the Beta 6 observation; the Xcode 26.6 aggregate is
summarized in the matrix above. Undefined is undefined, across targets and
toolchain revisions, which is the conclusion those cases exist to support.
