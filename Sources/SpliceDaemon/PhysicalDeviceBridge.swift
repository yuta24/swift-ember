...
final class PhysicalDeviceBridge: @unchecked Sendable {
    ...
    func writeSession(token: String, buildIdentity: String, buildUUIDs: [String]) throws {
        ...
        let session = Session(protocolVersion: SpliceProtocol.version, token: token,
                              buildIdentity: buildIdentity, buildUUIDs: buildUUIDs)
        let local = workDirectory.appendingPathComponent("splice-session.json")
        try FileManager.default.createDirectory(at: workDirectory, withIntermediateDirectories: true)
        try JSONEncoder.sorted.encode(session).write(to: local, options: .atomic)
        try copyToDevice(source: local, destination: "Documents/splice-session.json")
    }
    ...
    func connectedProcess() -> Int32? {
        ...
        guard let result = try? runner("/usr/bin/xcrun", copyFromArguments(
            source: "Documents/splice-device-status.json", destination: local.path)),
              result.exitCode == 0,
              let data = try? Data(contentsOf: local),
              let status = try? JSONDecoder().decode(DeviceStatus.self, from: data),
              status.protocolVersion == SpliceProtocol.version,
    ...
    func requestLoad(_ request: LoadPatchRequest) async throws -> Reply {
        ...
        defer { try? FileManager.default.removeItem(at: stagingRoot) }
        let staging = stagingRoot
            .appendingPathComponent("SpliceRequests", isDirectory: true)
        try FileManager.default.createDirectory(at: staging, withIntermediateDirectories: true)
        ...
        try copyToDevice(source: staging, destination: "Documents/SpliceRequests")
        ...
        for _ in 0..<40 {
            let result = try runner("/usr/bin/xcrun", copyFromArguments(
                source: "Documents/SpliceResponses/\(requestID).json", destination: response.path))
            if result.exitCode == 0,
               let data = try? Data(contentsOf: response),
               let reply = try? JSONDecoder().decode(Envelope.self, from: data) {
                guard reply.protocolVersion == SpliceProtocol.version else {
                    throw SpliceError(stage: .load, subject: requestFile.lastPathComponent,
                                      reason: "the runtime speaks protocol \(reply.protocolVersion) and the daemon speaks \(SpliceProtocol.version)",
                                      recovery: .rebuild)
                }
    ...
    private func signAndVerify(_ image: URL) throws {
        guard let identity = context.codeSigningIdentity, !identity.isEmpty else {
            throw SpliceError(stage: .link, subject: image.lastPathComponent,
    ...
    private func teamIdentifier(of path: String) throws -> String {
        ...
        guard result.exitCode == 0 else {
            throw SpliceError(stage: .link, subject: (path as NSString).lastPathComponent,
    ...
    private func copyToDevice(source: URL, destination: String) throws {
        ...
        guard result.exitCode == 0 else {
            throw SpliceError(stage: .transfer, subject: source.lastPathComponent,
