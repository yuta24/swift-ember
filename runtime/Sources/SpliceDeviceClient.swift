#if SPLICE_ENABLED && os(iOS) && !targetEnvironment(simulator) && !targetEnvironment(macCatalyst)
...
final class SpliceDeviceClient: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "dev.swift-splice.device-runtime")
    ...
    private func refreshSession() {
        let url = documents.appendingPathComponent("splice-session.json")
    ...
    private func processRequests() {
        let requests = documents.appendingPathComponent("SpliceRequests", isDirectory: true)
        let responses = documents.appendingPathComponent("SpliceResponses", isDirectory: true)
    ...
    private func writeStatus(for session: Session) {
        ...
        let url = documents.appendingPathComponent("splice-device-status.json")
        try? data.write(to: url, options: .atomic)
    }
}
