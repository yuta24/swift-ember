...
public struct PatchableDeclaration: Sendable {
    ...
    /// another file can shadow the package's `.emberable()` and return
...
public struct FileIndex: Sendable {
    ...
    public let declaresEmberable: Bool
...
public enum DeclarationIndexer {
    public static func index(source: String) -> FileIndex {
        ...
        let importsEmberSwiftUI = importsEmberSwiftUIDirectly(file)
        let declaresEmberable = EmberableDeclarationFinder.found(in: file)
        let observedContexts = observeEmberContexts(
            in: file.statements.map(\.item), context: [])

        walk(members: file.statements.map(\.item), context: [],
             importsSwiftUI: importsSwiftUI,
             importsEmberSwiftUI: importsEmberSwiftUI,
             declaresEmberable: declaresEmberable,
    ...
    private static func walk(members: [CodeBlockItemSyntax.Item], context: [String],
                             importsSwiftUI: Bool,
                             importsEmberSwiftUI: Bool,
                             declaresEmberable: Bool,
    ...
    private static func visit(_ decl: DeclSyntax, context: [String],
        ...
        if let variable = decl.as(VariableDeclSyntax.self) {
            record(variable: variable, context: context,
                   ...
                   importsEmberSwiftUI: importsEmberSwiftUI,
                   declaresEmberable: declaresEmberable,
                   observesEmber: observedContexts.contains(context.joined(separator: ".")),
                   into: &builder)
            return
        }
    ...
    /// may never evaluate.
    private static func observeEmberContexts(
        ...
        for item in members {
            ...
            if declarations.contains(where: { declaration in
                ...
                return variable.attributes.contains { element in
                    ...
                    return name == "ObserveEmber"
                        || name == "EmberSwiftUI.ObserveEmber"
                }
            }) {
                found.insert(path.joined(separator: "."))
            }
            found.formUnion(observeEmberContexts(
    ...
    ///
    /// A spelling match on `@ObserveEmber` and `.emberable()` alone is not
    ...
    private static func importsEmberSwiftUIDirectly(_ file: SourceFileSyntax) -> Bool {
        file.statements.contains { item in
            ...
            return node.importKindSpecifier == nil
                && node.path.count == 1
                && node.path.first?.name.text == "EmberSwiftUI"
        }
    }

    private final class EmberableDeclarationFinder: SyntaxVisitor {
        private var seen = false

        static func found(in file: SourceFileSyntax) -> Bool {
            let finder = EmberableDeclarationFinder(viewMode: .sourceAccurate)
            finder.walk(file)
            return finder.seen
        }

        override func visit(_ node: FunctionDeclSyntax) -> SyntaxVisitorContinueKind {
            if node.name.text == "emberable" { seen = true }
    ...
    private static func record(variable: VariableDeclSyntax, context: [String],
        ...
        if containsOpaqueType(declaredType) {
            ...
            guard erasedView,
                  ...
                  !declaresEmberable,
                  observesEmber,
                  endsInEmberable(accessors)
            else {
                let reason: String
                if erasedView, name == "body", endsInEmberable(accessors),
                   declaresEmberable {
                    reason = swiftUIShadowReason
                } else if erasedView, name == "body", endsInEmberable(accessors),
                          !importsEmberSwiftUI {
    ...
    private static func endsInEmberable(_ accessors: AccessorBlockSyntax) -> Bool {
        ...
        return member.declName.baseName.text == "emberable"
    }
    ...
    /// as `label()` becoming `ember_g1_label____()` --- and `#fileID` to the
    ...
    static let swiftUIBodyReason = """
        ...
        List (measured on iOS 26 and later). Add `@ObserveEmber` to the View \
        and make `.emberable()` the body's outermost expression, then rebuild
        """

    static let swiftUIObserverReason = """
        erases its body with `.emberable()` but the enclosing View has no \
        `@ObserveEmber` property in this file, so a loaded body is not guaranteed \
        to be evaluated. Add the observer and rebuild
        """

    static let swiftUIImportReason = """
        looks like a SwiftUI ember boundary but the file does not import \
        `EmberSwiftUI` as a whole module, so the classifier cannot prove those \
        names are this package's opt-in API. Import `EmberSwiftUI` and rebuild
        """

    static let swiftUIShadowReason = """
        uses `.emberable()` in a source file that also declares that name. \
