...
-   `override` method and computed-property body change, including a body
    that calls `super`, and including one reached through `objc_msgSend`
    rather than Swift's vtable. The replacement is a separate declaration
    bound to the original's key, so an extension accepts it although an
    override could not go there,
-   a declaration added by the edit --- a new helper function, a new method,
